...
| ---- | ---- | ----------- |
| `steganographer-core/` | Crate | Pure algorithms: generic packets/carriers, LSB video/audio, crypto, overlays, signing, metrics, config, frequency-domain kernels, encryption, ECC, multi-frame, adaptive, hash-chain, KDF, password KDF, transforms, and steganalysis (27 modules, 264 unit tests, 113 integration tests) |
...
- **Tests**: 264 core unit + 113 core integration + 6 CLI unit + 26 CLI integration + 23 dashboard + 2 GStreamer/doc = **434 passing tests**
...
cargo test -p steganographer-core              # 377 tests (264 unit + 113 integration, Ed25519 default)
cargo test -p steganographer-core --features ethereum  # includes Ethereum tests
cargo test --workspace                         # 434 total tests
