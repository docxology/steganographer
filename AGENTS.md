...
| ---- | ---- | ----------- |
| `steganographer-core/` | Crate | Pure algorithms: generic packets/carriers, LSB video/audio, crypto, overlays, signing, metrics, config, frequency-domain kernels, encryption, ECC, multi-frame, adaptive, hash-chain, KDF, password KDF, and steganalysis (26 modules, 253 unit tests, 113 integration tests) |
...
- **Tests**: 253 core unit + 113 core integration + 6 CLI unit + 22 CLI integration + 23 dashboard + 2 GStreamer/doc = **419 passing tests**
...
cargo test -p steganographer-core              # 366 tests (253 unit + 113 integration, Ed25519 default)
cargo test -p steganographer-core --features ethereum  # includes Ethereum tests
cargo test --workspace                         # 419 total tests
