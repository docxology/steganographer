...
- **Doc/CI accuracy sweep.** The test-count badge and every test summary
  ...
  count — 311 claimed versus 434 actually passing (core 377 = 264 unit + 113
  integration, CLI 32, dashboard 23, gst 2). The CI badge job counted
...
- **Generic packet transform pipeline (`PKT-004` slice).** New
  ...
  Reed-Solomon error correction to a generic packet body, records the ordered
  `TransformDescriptor`s in the envelope, and sets the `FLAG_PAYLOAD_SIGNED` /
  `FLAG_COMPRESSED` / `FLAG_ENCRYPTED` / `FLAG_ERROR_CORRECTED` locator flags.
  The sign transform records the public key + signature (96 bytes) over the
  logical payload so a decoder can attribute it to an identity; the AEAD
  ...
  reuses a nonce. Chunked RS lifts the 255-symbol codeword ceiling so arbitrary
  payloads are covered, and compression is recorded only when it actually
  shrinks the payload. `encode --signing-key/--compress/--encrypt/--ecc` and
  `decode --decrypt` now round-trip transformed generic packets; unknown
  critical transforms, missing decryption keys, and invalid signatures fail
  closed.
